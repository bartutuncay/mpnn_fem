## varying BC supports - WArped shape - UNiform load
...
load_type = np.random.choice(['bending','shear','torsion','axial'])
...
zmin = nodes[:, 2].min()
zmax = nodes[:, 2].max()
total_twist_deg = np.random.uniform(-30.0, 30.0)
total_twist_rad = torch.tensor(total_twist_deg * np.pi / 180.0, dtype=nodes.dtype)
x = nodes[:, 0]
y = nodes[:, 1]
z = nodes[:, 2]
...
if support_loc == 'clamped':
    model.displacements[on_z_top] = 0
    model.constraints[on_z_top] = True
    force_vector[on_z_top, 0] = 0
    force_vector[on_z_top, 1] = 0
    force_vector[on_z_top, 2] = 0
if support_loc == 'simply_supported':
    model.displacements[on_z_top] = 0
    support_pin = on_z_zero & on_z_support
    support_roller = on_z_top & on_z_support
    model.constraints[support_pin,:] = True
    model.constraints[support_roller,1] = True
    force_vector[on_z_top, 0] = 0
    force_vector[on_z_top, 1] = 0
    force_vector[on_z_top, 2] = 0
...
torch.save(
    ...
    f"../../../scratch/btuncay/gnn/ablation_datasets/var_bc/warped/uniform/sim_{idx}.pt",
)
