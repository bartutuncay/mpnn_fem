...
class GEN_Multiscale(torch.nn.Module):
    ...
    def forward(self,x,edge_index,edge_attr,edge_index_coarse,edge_attr_coarse,edge_index_diff,edge_attr_diff,edge_index_aggr,edge_attr_aggr,batch):
        x = self.proj(x)
        ea_w = (edge_attr**2).sum(-1,keepdim=True)
        ea_w_coarse = (edge_attr_coarse**2).sum(-1,keepdim=True)
        ea_w_diff = (edge_attr_diff**2).sum(-1,keepdim=True)
        ea_w_aggr = (edge_attr_aggr**2).sum(-1,keepdim=True)
        edge_attr = self.edge_proj(ea_w)
        edge_attr_coarse = self.coarse_edge_proj(ea_w_coarse)
        edge_attr_diff = self.diff_edge_proj(ea_w_diff)
        edge_attr_aggr = self.aggr_edge_proj(ea_w_aggr)
        # fine pass (initial)
...
def compute_losses(batch, pred_u, pred_f, y_u, y_f):
    ...
    Lg = e2_g / (y2_g + eps)
    L_u = Lg.mean() # deformation loss term
    
    pred_u_bc = pred_u*bc
    y_u_bc = y_u*bc
    
    e_bc = pred_u_bc - y_u_bc
    L_u_bc = (e_bc.pow(2).sum(dim=1).mean()) / (y_u_bc.pow(2).sum(dim=1).mean() + eps) # deformation loss term for constrained nodes

    fext   = batch.x[:,3:6]
    pred_f = pred_f*(1-bc) + 0*bc
    L_fint = F.mse_loss(pred_f, y_f) # reaction force loss term
    L_eq   = F.mse_loss(((torch.ones_like(bc)-bc)*(pred_f - fext)).sum(),torch.zeros((),device=bc.device)) # equilibrium loss term

    dir_loss = dirichlet_loss(pred_u,batch.edge_index) # dirichlet loss term
...
## Training Loop
model = GEN_Multiscale(in_channels=6,edge_in=1,layers=4,layers_coarse=12,latent_dim=128,out_channels=3).to(device)

alias = f"equivariant_{sim_dataset}"
print(alias)
os.makedirs(f"training/equivariant/{alias}",exist_ok=True)
...
for epoch in range(1, EPOCHS + 1):
    ...
    if epoch == 1 or epoch % 100 == 0:
        torch.save(model.state_dict(), f"training/equivariant/{alias}/weights_{epoch}.pt")

print("Best val:", best_val)

pd.DataFrame(loss_records).to_csv(f"training/equivariant/{alias}/losses.csv", index=False)
