# Multi-Scale Model with Spherical Coordinates and Periodic Activation Function
...
print(alias)
os.makedirs(f"training/spherical_coords_sin/{alias}",exist_ok=True)
...
for epoch in range(1, EPOCHS + 1):
    ...
    if epoch == 1 or epoch % 200 == 0:
        torch.save(model.state_dict(), f"training/spherical_coords_sin/{alias}/weights_{epoch}.pt")

print("Best val:", best_val)

pd.DataFrame(loss_records).to_csv(f"training/spherical_coords_sin/{alias}/losses.csv", index=False)
