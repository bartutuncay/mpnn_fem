# Multi-Scale Model with tanh Activation Functions
...
## Training Loop
model = GEN_Multiscale(in_channels=6,edge_in=3,layers=2,layers_coarse=12,latent_dim=128,out_channels=3).to(device)

alias = f"multiscale_tanh_{sim_dataset}"
print(alias)
os.makedirs(f"training/multiscale_tanh/{alias}",exist_ok=True)
...
for epoch in range(1, EPOCHS + 1):
    ...
    if epoch == 1 or epoch % 200 == 0:
        torch.save(model.state_dict(), f"training/multiscale_tanh/{alias}/weights_{epoch}.pt")

print("Best val:", best_val)

pd.DataFrame(loss_records).to_csv(f"training/multiscale_tanh/{alias}/losses.csv", index=False)
