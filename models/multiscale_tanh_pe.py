# Multi-Scale Model with tanh Activation Functions and Positional Encoding
...
## Training Loop
model = GEN_Multiscale(in_channels=10,edge_in=3,layers=2,layers_coarse=12,latent_dim=128,out_channels=3).to(device)

alias = f"multiscale_tanh_pe_{sim_dataset}"
print(alias)
os.makedirs(f"training/multiscale_tanh_pe/{alias}",exist_ok=True)
...
for epoch in range(1, EPOCHS + 1):
    ...
    if epoch == 1 or epoch % 200 == 0:
        torch.save(model.state_dict(), f"training/multiscale_tanh_pe/{alias}/weights_{epoch}.pt")

print("Best val:", best_val)

pd.DataFrame(loss_records).to_csv(f"training/multiscale_tanh_pe/{alias}/losses.csv", index=False)
