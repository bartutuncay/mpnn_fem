# Multi-Scale Model with Spherical Coordinates
...
from datasets_src.dataloader_spherical import make_loader
import os
import argparse
...
alias = f"spherical_coords_tanh_{sim_dataset}"
print(alias)
os.makedirs(f"training/spherical_coords_tanh/{alias}",exist_ok=True)
...
for epoch in range(1, EPOCHS + 1):
    ...
    if epoch == 1 or epoch % 200 == 0:
        torch.save(model.state_dict(), f"training/spherical_coords_tanh/{alias}/weights_{epoch}.pt")

print("Best val:", best_val)

pd.DataFrame(loss_records).to_csv(f"training/spherical_coords_tanh/{alias}/losses.csv", index=False)
